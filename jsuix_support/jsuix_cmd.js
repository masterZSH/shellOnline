...
function commandHelp(env) {
    env.stdin = new VfsFileHandle(new VfsFile('p', [
        '%+r System Help %-r',
        'zsh/shell is a virtual OS in UN*X-style for JavaScript and web-browsers.',
        '',
        ...
        ' invaders   the "space invaders" arcade game for zsh/shell',
...
function commandReset(env) {
    if (env.status == '') {
        ...
        cnslType('y');
        newLine();
        cnslType('halting system for reboot ...');
...
function commandHello(env) {
    ...
    var s = ' ' + os_version + t + '\n';
    s += ' by mass:werk - media environments <https://shell.zhoudapao.cn>\n';
...
function commandSplitScreen(env) {
    ...
    cnslClear();
    if (split) {
        cnslType('split mode on', 1);
        newLine();
        cnslTypeAt(conf_rows - 2, 0, '--------------------------------------------------------------------------------');
        cnslTypeAt(conf_rows - 1, 0, 'zsh/shell by mass:werk. type "splitmode off" or "clear" to return to normal mode.');
...
function commandFeatures(env) {
    env.stdin = new VfsFileHandle(new VfsFile('p', [
        '%+r System Features %-r',
        os_greeting,
        ' by mass:werk - media environments; (c) N.Landsteiner <https://shell.zhoudapao.cn>',
        ' ',
        ' zsh/shell is a virtual terminal and UN*X-like operating system for web browsers.',
...
function manPrintAll() {
    ...
    s += '\n\n(c) mass:werk 2003; <https://shell.zhoudapao.cn>';
    var w = window.open();
...
function cmdSetFDump(win) {
    ...
    for (var i = 0; i < dirs.length; i++) {
        ...
        if (typeof d == 'object') {
            if (d.kind == 'd') {
                cnslType('- dir  "' + fn + '" already exists, skipped.');
                newLine()
            } else {
                cnslType('- dir  "' + fn + '" failed, file with same name already exists.');
                newLine()
            }
        } else {
            d = vfsCreate(fn, 'd', mode, md);
            if (typeof d == 'object') {
                cnslType('- dir  "' + fn + '" created.');
                newLine()
            } else {
                cnslType('- dir  "' + fn + '" failed.');
                newLine()
            }
        }
    };
    for (var i = 0; i < files.length; i++) {
        ...
        if (typeof f == 'object') {
            if (f.kind == 'd') {
                cnslType('- file "' + fn + '" failed, directory with same name already exists.');
                newLine()
            } else {
                cnslType('- file "' + fn + '" already exists, skipped.');
                newLine()
            }
        } else {
            ...
            if (typeof f == 'object') {
                f.lines = lines;
                cnslType('- file "' + fn + '" created.');
                newLine()
            } else {
                cnslType('- file "' + fn + '" failed.');
                newLine()
            }
        }
    };
...
function commandHomeImport(env) {
    if (env.status == '') {
        cnslType('please insert file-dump in the opening form.');
        newLine();
        cnslType('type "c" to abbort ...');
        newLine();
        ...
        cnslType('import terminated by user.');
        newLine();
