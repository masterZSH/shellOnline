...
// all rights reserved
// zsh/shell is NOT a free software (see faq in history.txt).
...
function krnlInit() {
    ...
    if (termGuiReady()) {
        ...
        cnslType('  starting up [init] ...');
        newLine();
        newLine();
        cnslType('  terminal (dhtml-gui) ready.');
        newLine();
        ...
        cnslType('ok');
        newLine();
        cnslType('  re-entering file-system as root.');
        newLine();
        ...
        cnslType('ok');
        newLine();
        cnslType('  trying for RC-file ... ');
        t_c = r_col - 3;
        if (self.jsuixRC) {
            cnslType('found');
            newLine();
            if ((self.jsuixRX) && (self.jsuixRX())) {
                cnslType('  rc-profile looks good.');
                newLine();
            } else {
                cnslType('# rc-profile seems to have syntactical problems,');
                newLine();
                cnslType('# system may hang, trying further ...');
                newLine()
            };
...
function krnlLogin(reenter) {
    ...
    if (reenter) {
        ...
        newLine();
        newLine();
        cnslType('re-login to system or type "exit" for shut down.');
        newLine();
        newLine();
        newLine();
        cnslType('         ZZZZZZZZZZZ   SSSSSS        H        H');
        newLine();
        cnslType('                 ZZ   SS      SS     H        H');
        newLine();
        cnslType('               ZZ     SS             H        H');
        newLine();
        cnslType('             ZZ        SSSSSS        HHHHHHHHHH');
        newLine();
        cnslType('          ZZ                SS       H        H');
        newLine();
        cnslType('        ZZ            SS    SS       H        H');
...
function krnlLoginDmn(user) {
    ...
    if (krnlCurPcs.user) {
        ...
        cnslType('  zsh/shell:Log-on - type user-name (e.g. "guest") and hit <return>.');
