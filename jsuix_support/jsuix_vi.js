...
function viBackup(env, m) {
    ...
    if ((m == 1) && (viUndoBuffer == null)) {
        viBeep(env, 'undo buffer empty');
        return
    } else if ((m == 2) && (viRedoBuffer == null)) {
        viBeep(env, 'redo buffer empty');
        return
    };
...
function viCommandHandler(env) {
    ...
    if ((env.cmd == 'q') || (env.cmd == 'quit') || (env.cmd == 'q!') || (env.cmd == 'quit!')) {
        ...
        if (!env.filename) {
            viCmdError(env, 'no filename specified');
            return
        } else if ((env.args[0] == 'view') && (!env.newname)) {
            viCmdError(env, 'opened in read only mode (write with new name)');
            return
        };
        var f = vfsOpen(env.filename, 2);
        if (f != 0) {
            if (f == -2) {
                viCmdError(env, env.filename + ': permission denied.');
                return
            } else if (f < 0) {
                viCmdError(env, env.filename + ':no write permission in parent directory.');
                return
            } else if (f.inode == krnlDevNull) {
                f.touch();
                viStatus(env, '/dev/null: 0 lines 0 characters')
            } else if (f.kind != 'f') {
                viCmdError(env, env.filename + ': not a plain file');
                return
            } else if ((env.newname) && (env.changed) && (!force)) {
                viCmdError(env, env.filename + ': allready exists (use ! to override)');
                return
            ...
            if (f < 0) {
                ...
                viCmdError(env, env.filename + ': directory not found.');
                return
            } else {
                env.changed = null;
                var chrs = 0;
